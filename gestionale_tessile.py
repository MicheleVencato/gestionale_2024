...
def salva_dati(df, sheet_name):
    try:
        ...
        # Imposta il foglio attivo se non esiste un foglio visibile
        if len([s for s in book.sheetnames if book[s].sheet_state == 'visible']) == 0:
            st.error("Errore: almeno un foglio deve essere visibile.")
            return

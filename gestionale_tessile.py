...
def inserisci_prodotto():
    ...
    fornitore_selezionato = st.selectbox("Seleziona Fornitore", fornitori_df['Nome Fornitore'])
...
# Funzione per inserire nuovi ordini con menu a tendina per clienti, prodotti e fornitori
def inserisci_ordine():
    st.write("## Inserisci Nuovo Ordine")
    id_ordine = st.text_input("ID Ordine")
    cliente_selezionato = st.selectbox("Seleziona Cliente", clienti_df['Nome'])
    prodotto_selezionato = st.selectbox("Seleziona Prodotto", prodotti_df['Descrizione'])
    fornitore_selezionato = st.selectbox("Seleziona Fornitore", fornitori_df['Nome Fornitore'])
    ...
    if st.button("Aggiungi Ordine"):
        nuovo_ordine = pd.DataFrame({
            "ID Ordine": [id_ordine],
            "ID Cliente": [cliente_selezionato],
            "ID Prodotto": [prodotto_selezionato],
            "ID Fornitore": [fornitore_selezionato],
...
# Funzione per inserire nuove fatture con menu a tendina per ordini, clienti, e fornitori
def inserisci_fattura():
    st.write("## Inserisci Nuova Fattura")
    id_fattura = st.text_input("ID Fattura")
    ordine_selezionato = st.selectbox("Seleziona Ordine", ordini_df['ID Ordine'])
    cliente_selezionato = st.selectbox("Seleziona Cliente", clienti_df['Nome'])
    fornitore_selezionato = st.selectbox("Seleziona Fornitore", fornitori_df['Nome Fornitore'])
